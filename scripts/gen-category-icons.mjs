#!/usr/bin/env node
/**
 * gen-category-icons.mjs — 生成足迹分类图标（lucide 字形 → 小程序 PNG）
 *
...
const PLAIN = ['sliders-horizontal', 'plus', 'search'];
...
function compose(inner, size, inset, withCircle) {
  ...
  const circle = withCircle
    ...
    : '';
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">${circle}<g transform="translate(${inset},${inset}) scale(${scale.toFixed(4)})" fill="none" stroke="${STROKE}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${inner}</g></svg>`;
}

const work = mkdtempSync(join(tmpdir(), 'fp-icons-'));

/** SVG 包进零边距 HTML，Chrome 无头按 window-size 精确出图（透明底） */
function rasterize(svgPath, outPng) {
...
function emit(name, inner, inset, withCircle) {
  const svgPath = join(work, `${name}.svg`);
  writeFileSync(svgPath, compose(inner, SIZE, inset, withCircle));
  emitFromSvg(name, svgPath);
}

function emitFromSvg(name, svgPath) {
...
for (const icon of PLAIN) {
  ...
  emit(`lucide-${icon}`, await lucideInner(icon), 0, false);
}
