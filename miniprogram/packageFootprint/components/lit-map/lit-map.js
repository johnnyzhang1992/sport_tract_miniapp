/**
 * 点亮地图 + 点亮城市（按省展开收起）
 * 从原「我的 → 点亮地图」页整体搬来，挂在数据统计页底部（主包页面走分包异步化引用本组件）。
 * 数据自取：/stats/footprint（省份计数 + 一份扁平城市）与 /geo/china-map（省界 GeoJSON）。
 * 页面下拉刷新时可外部调 load() 重画。
 */
...
Component({
  ...
  lifetimes: {
    ready() {
      // canvas 就绪后建图；数据可能比这更早/更晚到，两条路径都做了 setOption 兜接
      this.initChart();
      this.load();
    },
  },
  ...
  methods: {
    /** 主图初始化（原页面 onReady 的等价位置） */
    initChart() {
      const comp = this.selectComponent('#mapCanvas');
      this._mapComp = comp;
    ...
    async load() {
      ...
      try {
        ...
        if (!this._chinaMap) {
          try {
            const map = await app.globalData.api.get('/geo/china-map');
            this._chinaMap = map;
        ...
        const [res, ov] = await Promise.all([
          app.globalData.api.get('/stats/footprint'),
          app.globalData.api.get('/stats/overview').catch(() => null),
        ]);
        // 累计轨迹数（分享图统计用）
        this._trackCount = ov && ov.total ? ov.total.count : 0;
        // 该接口的 provinces[].cities 是市个数，城市在扁平 cities 里，先挂回去再出行视图
        this._provinces = regionList.attachCities(res.provinces || [], res.cities || []);
        this.setData({
          provinceCount: res.provinceCount,
          cityCount: res.cityCount,
          regionRows: regionList.buildRegionRows(this._provinces, this.data.expandedRegions),
          regionSummary: regionList.regionSummaryText(this._provinces),
        });
        // 更新地图高亮
        const mapData = (res.provinces || []).map((p) => ({ name: p.name, value: p.count }));
        this._mapData = mapData;
        if (this.chart) this.chart.setOption(getMapOption(mapData));
        if (this.fsChart) this.fsChart.setOption(getMapOption(mapData));
      } catch (e) {
    ...
    openProvinceMap(name, code) {
      // 无点亮城市也允许打开：全部城市灰显
      const prov = (this._provinces || []).find((p) => p.name === name);
      const provCities = prov ? prov.cities : [];
    ...
    /** 导出地图图片 → 预览弹窗（导出本身走共享 util map-image；loading 与兜底 toast 留在组件） */
